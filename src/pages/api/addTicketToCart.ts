...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        console.log('Preparing to send request to external API for ticket ID:', ticketId);
        try {
            const apiResponse = await axios.post(`https://api.sunsetcinema.in-create.online/api/en/cart/ticket/${ticketId}`, {}, {
            ...
            if (error.response) {
                ...
                res.status(error.response.status).json({ message: 'Failed to add ticket to cart', details: error.response.data });
            } else {
