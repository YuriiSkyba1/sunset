...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	if (req.method === "PUT") {
		const { snackItem, storeItemQuantity } = req.body;
		console.log(
			`Preparing to send request to external API for setting the quantity ${storeItemQuantity} of store item ID: ${snackItem}`
		);
		try {
			const apiResponse = await axios.put(
				`https://api.sunsetcinema.in-create.online/api/en/cart/store-item/${snackItem}/${storeItemQuantity}`,
				{},
				{
