...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	if (req.method === "POST") {
		const { payment_method, name, email, phone, accept_nawsletter } = req.body;
		console.log("Preparing to send request create order: ", payment_method, name, email, phone, accept_nawsletter);
		try {
			const apiResponse = await axios.post(
				`https://api.sunsetcinema.in-create.online/api/en/checkout`,
				{ payment_method, name, email, phone, accept_nawsletter },
				{
