...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	if (req.method === "GET") {
		console.log("Preparing to send request to external API for showing checkout:");
		try {
			const apiResponse = await axios.get(`https://api.sunsetcinema.in-create.online/api/en/checkout`, {
			...
			if (error.response) {
				...
				res.status(error.response.status).json({
					message: "Failed to show checkout",
